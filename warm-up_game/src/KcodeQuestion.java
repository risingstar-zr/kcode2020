...
public class KcodeQuestion {
    ...
    /* 线程池参数 
    CountDownLatch await() cutdown() 
           1、线程池核心线程的数量；2、线程池可创建的最大线程数量；4、指定了keepAliveTime的单位，可以为毫秒，秒，分，小时等
           5、存储未执行的任务的队列；6、创建线程的工厂，如果未指定则使用默认的线程工厂；7、指定了当任务队列已满，并且没有可用线程执行任务时对新添加的任务的处理策略；
    */
    ...
    public void prepare(InputStream inputStream) throws IOException {
        ...
        long fileSize = fileChannel.size();//B-KB-Mb-GB
        int mapOffset = 0;
...
//            if(startTime ==1587987931 && methodName.equals("getUserName7") && elapsedTime==29) {
...
//            }
        ...
        // 阻塞一个线程，等待其它线程先后到达某个条件的时候，再执行这个线程的后续操作
